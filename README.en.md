...
### FGTS value updates with official indices, for AI

**FGTS calculation** for AI. Update FGTS amounts and differences with official correction and interest over the period. Live indices from public sources, no login.

- 🏦 **Updates FGTS amounts** and differences with correction and interest
...
**Manual** (if the deeplink does not open): [claude.ai/customize/connectors](https://claude.ai/customize/connectors?surface=cowork) → **+** → **Add custom connector** → paste **Name** `Cálculo de FGTS` and **URL** `https://api.mcp.ai/p_calculo-fgts`.

### Cursor

[➕ Install Cálculo de FGTS in Cursor](cursor://anysphere.cursor-deeplink/mcp/install?name=calculofgts&config=eyJ1cmwiOiJodHRwczovL2FwaS5tY3AuYWkvcF9jYWxjdWxvLWZndHMifQ==)

### VS Code (Copilot Chat)

[➕ Install Cálculo de FGTS in VS Code](vscode:mcp/install?name=calculofgts&config=%7B%22type%22%3A%22http%22%2C%22url%22%3A%22https%3A%2F%2Fapi.mcp.ai%2Fp_calculo-fgts%22%7D)
...
```
Update R$12,000 of FGTS from 2019 to today with correction and interest
Correct the FGTS difference for this period by the official index
```
...
MIT — see [LICENSE](LICENSE). The MCP server at `api.mcp.ai/p_calculo-fgts` is proprietary (hosted); this repo (manifests, docs, skills) is MIT.
